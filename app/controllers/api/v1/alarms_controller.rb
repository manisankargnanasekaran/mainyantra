...
#   @alarms = Alarm.limit(15).where(machine_id:Tenant.find(params[:tenant_id]).machines.ids).distinct(:alarm_message).order("updated_at DESC")
    @alarms = Alarm.where(machine_id:Tenant.find(params[:tenant_id]).machines.ids).distinct(:alarm_message).order("updated_at DESC").where('updated_at > ?', 7.days.ago)
#@alarms = Alarm.all
    render json: @alarms
  end
