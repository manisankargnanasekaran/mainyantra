...
class MachineLog < ApplicationRecord
 ...
 if params[:shifttransaction_id].present? && params[:shifttransaction_id] != "0"
  ...
  if shift.present?
    ...
    if  params[:date].present? ? params[:date] : Date.today.to_s
      ...
      # machine_log = machine.machine_logs.where("created_at >= ? AND created_at <=?",start_time,end_time).order(:id)
       machine_log = machine.machine_daily_logs.where("created_at >= ? AND created_at <=?",start_time,end_time).order(:id)
    end
 ...
 total_shift_time_available_for_downtime =  Time.now - start_time
                  ...
                  total_run_time = 0
                   ...
                   job_name = "O"+j_name
                      if machine_log.where.not(parts_count:"-1").where(programe_number:j_name).count != 0 
                        #if machine_log.where.not(parts_count:"-1").where(:programe_number=>j_name).pluck(:parts_count).uniq.include?("0")
    ...
    target_parts_total=MachineDailyLog.target_parts(params) #machine model target parts 
      data = {
        :shifts=>all_shift,
        ...
        :utilization=>utilization.round(),
         #----------traget parts-----#
        :efficiency=>target_parts_total[:efficiency],
        :quality=>100,
        :oee_report=>((utilization*target_parts_total[:efficiency]*100)/10000).round(),
        #-------------#
        :shift_no=>shift_no,
...
def self.stopage_time_details(params)
    ...
    #latest code for date check
     if params[:date].present? && params[:date] == "undefined" || params[:date] == Date.today.to_s
   ...
   # total_data = machine.machine_daily_logs.where(created_at: start_time..end_time).order(:id)
    ...
    unless total_data.count == 0
      ...
      rr = bb.map{|ll| ll.flatten}
        ...
        rr.each_with_index do |ff,index|
          unless index+1 == rr.count
            if rr[index+1][1] != "100" && ff[1] != "100"
               run_additional << ((rr[index+1][2] - ff[-2])*60 + (rr[index+1][3]/1000-ff[-1]/1000))
               ideal_additional << (rr[index+1][0].to_time - ff[4].to_time) - ((rr[index+1][2]-ff[-2])*60 + (rr[index+1][3]/1000-ff[-1]/1000))
            end
          end
        end
        
        run_additional = run_additional.reject{|i| i < 0}
        ideal_additional = ideal_additional.reject{|i| i < 0}
      ...
      hourinterval.map do |hr|
        ...
        hrdowntime = hrdata.map{|mclog| mclog.machine_status == "3" ? "***" : mclog.created_at }
        downtime = hrdowntime.split("***").map{|dwntime| (dwntime[-1] - dwntime[0]).to_i/60 unless dwntime.empty?}.compact.sum
        machine_details[:downtime] << {"time": hr.strftime("%I:00 %p"), "downtime": downtime}
        machine_details[:production] << {"time": hr.strftime("%I:00 %p"), "parts_count": parts_count}
        firsthr = hr
      end

      machine_details[:total_run_time]=Time.at(machine_details[:machine_status_report].select{|ll| ll[:status]=="3"}.map{|kk| a= kk[:time_difference_seconds]}.sum+run_additional.sum).utc.strftime("%H:%M:%S")
      machine_details[:total_ideal_time]=Time.at(machine_details[:machine_status_report].select{|ll| ll[:status] !="3" && ll[:status]!="100" }.map{|kk| a= kk[:time_difference_seconds]}.sum+ideal_additional.sum).utc.strftime("%H:%M:%S")
      ...
      machine_details[:remaining_time]=Time.at((end_time-start_time)-(Time.parse(machine_details[:total_run_time]).seconds_since_midnight+Time.parse(machine_details[:total_ideal_time]).seconds_since_midnight+Time.parse(machine_details[:total_stop_time]).seconds_since_midnight)+run_additional.sum+ideal_additional.sum).utc.strftime("%H:%M:%S")
...
def self.stopage_time_details(params)
   ...
   # end
    ...
    if params[:date].present? && params[:date] == "undefined" || date == Date.today.to_s
     ...
     #  total_data = machine.machine_monthly_logs.where(created_at: start_time..end_time).order(:id)
       total_data = machine.machine_daily_logs.where(created_at: start_time..end_time).order(:id)
    else
      total_data = machine.machine_logs.where(created_at: start_time..end_time).order(:id)
    end
    ...
    unless total_data.count == 0
      ...
      #  hrdowntime = hrdata.map{|mclog| mclog.machine_status == "3" ? "***" : mclog.created_at }
       ...
       # machine_details[:downtime] << {"time": hr.strftime("%I:00 %p"), "downtime": downtime}
       # machine_details[:production] << {"time": hr.strftime("%I:00 %p"), "parts_count": parts_count}
        machine_details[:downtime] << {"time": hr.strftime("%H:%M %p"), "downtime": downtime}
        machine_details[:production] << {"time": hr.strftime("%H:%M %p"), "parts_count": parts_count}
        machine_details[:runtime]  << {"time": hr.strftime("%H:%M %p"), "runtime": runtime}
        machine_details[:stoptime] <<  {"time": hr.strftime("%H:%M %p"), "stoptime": stoptime}
        firsthr = hr
      end
...
def self.hour_wise_status(params)
    date = params[:date].present? && params[:date] != "undefined"  ? params[:date] : Date.today.to_s
    machine = Machine.find_by(params[:machine_id])
    if params[:shifttransaction_id].nil? || params[:shifttransaction_id] == "null" || params[:shifttransaction_id] == "0" || params[:shifttransaction_id] == "undefined"
      start_time = (date+" "+machine.tenant.shift.day_start_time).to_time
      end_time = (date+" "+machine.tenant.shift.day_start_time).to_time + 1.day
      #time_range = start_time..end_time
   ...
   if params[:date].present? && params[:date] == "undefined" || params[:date] == Date.today.to_s
       ...
       machine_log = machine.machine_monthly_logs.where(created_at: start_time..end_time).order(:id)
    else
       machine_log = machine.machine_logs.where(created_at: start_time..end_time).order(:id)
    end
    ...
    bb = [] 
      ...
      #-------parts count hr -----------#
        start_parts_count1 = total_data.where.not(parts_count:"-1").first.parts_count if total_data.where.not(parts_count:"-1").present?
    ...
    unless end_time == hour
      data = {
        :hour => hour == hour_split.last ? hour.strftime("%I:%M %p")+"-"+(end_time).strftime("%I:%M %p") : hour.strftime("%I:%M %p")+"-"+(hour+1.hour).strftime("%I:%M %p"),#hour end time display
...
def self.hour_wise_status(params)
    ...
    if params[:date].present? && params[:date] == "undefined" || params[:date] == Date.today.to_s || params[:date] == ""
     ...
     #  machine_log = machine.machine_monthly_logs.where("created_at >= ? AND created_at <= ?",start_time,end_time).order(:id)
          machine_log = machine.machine_daily_logs.where(created_at: start_time..end_time).order(:id)
    else
       machine_log = machine.machine_logs.where(created_at: start_time..end_time).order(:id)
    end
...
if params[:report_type] == "Shiftwise"
  ...
  shifts.map do |shift|
    if shift.shift_start_time.include?("PM") && shift.shift_end_time.include?("AM")
     ...
     end_time_for_ideal = Time.now < end_time ? Time.now : end_time #for idle_time validate (26/12/17)
          total_shift_time_available = Time.parse(shift.actual_working_hours).seconds_since_midnight
             machines.order(:id).map do |mac|
              ...
              # machine_log = mac.machine_logs.where("created_at >= ? AND created_at <= ?",start_time,end_time).order(:id)
               total_shift_time_available_for_downtime =  Time.now - start_time #recent change for idle_time
                ...
                unless machine_log.present?
                 ...
                 time_difference = (machine_log.first.created_at.to_time.seconds_since_midnight - shift.shift_start_time.to_time.utc.seconds_since_midnight)/60
                  ...
                  total_shift_time_available_for_downtime = total_shift_time_available_for_downtime < 0 ? total_shift_time_available_for_downtime*-1 : total_shift_time_available_for_downtime # for idle_time 
                  total_run_time = Machine.calculate_total_run_time(machine_log)#Reffer Machine model                    
                   ...
                   total_shift_time_available = ((total_shift_time_available/60).round())*60
                    downtime =  ((end_time_for_ideal-start_time)-total_run_time).round() # for idle_time (26/12/17)
                  # downtime = (total_shift_time_available_for_downtime - total_run_time).round()#recent change
                 ...
                 # idle_time = downtime - total_load_unload_time
                  idle_time = downtime - total_load_unload_time < 0 ? 0 : downtime - total_load_unload_time#recent change
                  total_run_time = total_run_time.nil? ? 0 : total_run_time
                  targeted_parts = targeted_parts.nil? ? 0 : targeted_parts.round()
                  controller_part = machine_log.where.not(parts_count:"-1").last.present? ? machine_log.where.not(parts_count:"-1").last.parts_count.to_i : 0
...
#          :cycle_time=> parts_count.to_i > 0  ? machine_log.where(:parts_count=>machine_log.last.parts_count.to_i - 1).present? ? Time.at(machine_log.where(:parts_count=>machine_log.last.parts_count.to_i - 1).last.run_time*60 + machine_log.where(:parts_count=>machine_log.last.parts_count.to_i - 1).last.run_second.to_i/1000).utc.strftime("%H:%M:%S") : 0 : 0,
          ...
          :programe_number=>machine_log.pluck(:programe_number).uniq.reject{|i| i == "0" || i == ""}.split(",").join(" | "),
          :job_description=>job_description.nil? ? "-" : job_description.split(',').join(" & "),
          :targeted_parts => targeted_parts,
         ...
         if machine_log.count !=0
          parts_count=[]
           ...
           tenant.shift.shifttransactions.map do |shift|
            ...
            shift_data << mac.machine_logs.where("created_at >= ? AND created_at <= ?",start_time,end_time).order(:id)
          end
          total_load_unload=[]
          targeted_parts = []
...
def self.hour_detail(params)
  ...
  unless total_data.count == 0
    ...
    #  total_run_time = total_run_time.nil? ? 0 :  total_run_time
      total_run_time = total_run_time.nil?  ? 0 : total_run_time > 0 ? total_run_time : 0 #checking for negative value
      total_stop_time = total_stop_time.nil? ? 0 : total_stop_time
      total_stop_chart << total_stop_time
      data = {
        :time_interval => minute.strftime("%H:%M:%S")+" - "+(minute+10.minutes).strftime("%H:%M:%S"),
...
def self.month_reports
    ...
    tenant_active=Tenant.where(id:210)
  start_date= (Date.today - 3)#.ing_of_month #- 1.month
  end_date= Date.today.end_of_day# - 1.month
       ...
       #       if shift.shift_start_time.include?("PM") && shift.shift_end_time.include?("AM")
        ...
        #            end_time = (date+" "+shift.shift_end_time).to_time        
        #      end
              end_time_for_ideal = Time.now < end_time ? Time.now : end_time # for idle_time (26/12/17)
        total_shift_time_available = Time.parse(shift.actual_working_hours).seconds_since_midnight
        machines.order(:id).map do |mac|
        machine_log = mac.machine_monthly_logs.where("created_at >= ? AND created_at <= ?",start_time,end_time).order(:id)
        #machine_log = mac.machine_logs.where("created_at >= ? AND created_at <= ?",start_time,end_time).order(:id)
        ...
        parts_last = (controller_part.to_i)
        operator_efficiency = (parts_count.to_i*100/targeted_parts).round() unless targeted_parts == 0
     ...
     path="#{Rails.root}/public/#{tenant.tenant_name+"-"+start_date.strftime("%B")+"-"+"lastone"}.csv"
