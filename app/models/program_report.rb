class ProgramReport < ApplicationRecord
  ...
  date="2018-08-23"
  tenants = Tenant.where(id: [210]).ids
  ...
  tenants.each do |tenant|
	...
	#shifts.each do |shift_id|
	 shift = Shifttransaction.find(67)
	 # shift = Shifttransaction.current_shift(tenant.id)
		...
		(start_time.to_i..end_time.to_i).step(3600) do |hour|
		  ...
		  machines.where(id: 88).order(:id).map do |mac|
  ...
  @alldata.each do |data|
	if HourReport.where(date:data[0],shift_no: data[4], time: data[1], machine_id:data[6], tenant_id:data[15]).present?
	HourReport.find_by(date:data[0],shift_no: data[4], time: data[1], machine_id:data[6], tenant_id:data[15]).update(date:data[0], time: data[1], hour: data[2], shift_id: data[3], shift_no: data[4], operator_id: data[5], machine_id: data[6], job_description: data[7], parts_produced: data[8], run_time: data[9], ideal_time: data[10], stop_time: data[11], time_diff: data[12], log_count: data[13], utilization: data[14],  tenant_id: data[15], all_cycle_time: data[16])
	else
  #hour_data = CncReport.last_hour_report(data[6],data[15])
  if HourReport.where(machine_id:data[6], tenant_id:data[15]).last.present?              
    last_shift_report = HourReport.where(machine_id:data[6], tenant_id:data[15]).last
    report_id = HourReport.where(machine_id:data[6], tenant_id:data[15]).last
    shift_data = HourReport.last_hour_report(last_shift_report.machine_id,last_shift_report.tenant_id)
